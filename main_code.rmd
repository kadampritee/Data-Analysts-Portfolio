---
title: "Bellabeat products analysis"
author: "pritee kadam"
date: "9/28/2021"
output:
  html_document:
...
WeightLogInfo <- subset(WeightLogInfo,select=-c(WeightPounds,Fat,IsManualReport,LogId))
head(WeightLogInfo)
```
...
```{r}
Sleep = Sleep %>% mutate(Sleep_Amount = case_when(TotalMinutesAsleep/60>=6.0 & TotalMinutesAsleep/60<=9.0   ~ "Good Sleep",
    TotalMinutesAsleep/60<6.0 ~ "Under Sleep", 
    TotalMinutesAsleep/60>9.0 ~ "Over sleep"))
...
```{r}
Activity = Activity %>% mutate(Sedentary_Time = case_when(SedentaryMinutes  626 ~ "Good",
...
```{r}
ggplot(data = Activity)+
  geom_col(mapping = aes(Sleep_Amount.x, y= Calories, fill =Sleep_Amount.x))+
  facet_grid(~DistanceCategory)+
  labs(title = "How much Calories burns affecting by Sleep Quality By Distance Categories")+
  theme(axis.text = element_text(angle = 45))
```
